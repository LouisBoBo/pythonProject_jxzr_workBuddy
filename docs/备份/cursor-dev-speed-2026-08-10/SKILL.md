...
- 截图 +「改成这种 / 1:1 / 复刻 / 照着做」：把【截图理解】当 UI 视觉规格，直接进入本 Skill；
  确认写码后原图会附带给 Cursor Cloud（images），Cloud 可直接看图，文字规格为辅
- 前端若已弹出「意图确认」且用户点了写码，按写码处理
...
- 仅范围不清时用选项卡（范围 + 还原精度），示例：

```text
:::cursor_dev_options
{"title":"按截图复刻，请确认","summary":"布局以截图为准","notes_placeholder":"其它备注（可选）","groups":[{"id":"ui_scope","label":"本轮改动范围","multi":true,"required":true,"options":[{"id":"dash_home","label":"复刻首页/仪表盘主区"},{"id":"sidebar_nav","label":"含侧边栏/顶栏"},{"id":"charts_visual","label":"图表类型与色块按截图"},{"id":"keep_stack","label":"沿用技术栈，仅视觉贴近"}]},{"id":"ui_fidelity","label":"还原精度","multi":false,"required":true,"options":[{"id":"visual_high","label":"高还原：布局/色块/图表类型尽量接近"},{"id":"layout_data","label":"布局+数据对上即可，风格可简化"}]}]}
:::
```
...
```text
:::cursor_dev_propose
{"requirement":"完整需求摘要（含验收点；已有仓写明沿用技术栈）","repo":"","ref":""}
:::
```

截图复刻时 `requirement` **必须**含：

1. **视觉布局（按位置）** — 顶行/中左/右侧各卡是什么  
2. **图表与控件类型** — 仪表盘/进度条/折线/环形/色块底/半透明浮层等  
3. **配色风格** — 绿/粉/橙等色块；写明「非默认白卡片模板」  
4. **禁止臆造** — 截图没有的模块不要写进需求  
5. **技术** — 可沿用栈与 ECharts，但样式须贴近截图  
6. **验收** — 布局比例、图表类型、配色接近截图  
...
| 上下文信号 | 行为 |
|-----------|------|
| 现场读取且非空 / 技术栈已锁定 | 确认沿用栈；**禁止**再出技术栈选项；只问本轮增量 |
| 新项目 / 确为空仓且无前序写码 | 选项卡收集技术栈 + 范围；repo 填已定仓 |
| 同一窗口续聊 | 沿用已定仓与已确认选项，勿重新选仓 |
| 截图 1:1 且栈已锁定 | 视觉规格优先；勿用「同风格」覆盖截图色块 |

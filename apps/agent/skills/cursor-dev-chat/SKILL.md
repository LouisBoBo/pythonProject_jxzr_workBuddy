...
description: >-
  ...
  例外：用户已贴截图并用红框/标注指出改动点（如「红框处去掉」）时，禁止再问「红框指哪」或出勾选卡确认位置；读【截图理解】后直接 propose。
  贴图后若拿不准是「按图局部修改」还是「复原/对齐截图」，必须追问（意图确认卡二选一），禁止瞎猜、禁止编造「红框指哪一块」勾选项。
...
> **例外**：截图 + 红框/标注局部改 → 见 **1b2**，禁止用勾选卡问标注位置。  
> **截图拿不准时**：前端会先弹意图确认（按图修改 vs 复原截图）；你方也禁止在无【用户意图·…】标记时臆造红框位置勾选项。  
...
| 红框/标注指出改动（红框处去掉/圈出这块删除…） | **标注即目标**：读【截图理解】；**禁止**再问「红框指哪」或出勾选卡；直接 `:::cursor_dev_propose` |
| 贴了图但话术含糊（只说改登录页/改界面，未说复原还是局部改） | **先追问**：等前端意图卡或用户点选「按图修改 / 复原截图」；禁止瞎猜、禁止编造区块勾选 |
...
- 用户已说「红框处去掉」或图上有红框时，再问「红框具体指哪」或用勾选卡确认标注位置
- 用户已贴图但意图未标明「按图改 / 复原」时，不追问却直接编造改动区块勾选项或瞎猜布局
...
### 1b2. 红框 / 标注局部改（优先于「先出选项卡」）

当用户贴了截图，并且话术含「红框 / 标注 / 圈出 / 框处去掉 / 删除这块」等：

- **不要**走上面的「1. 先出选项确认卡」
- **不要**让用户勾选「红框指哪」——标注位置以【截图理解】为准（视觉模型已读图）
- 正文最多 1 句复述改动点，然后直接 `:::cursor_dev_propose`（默认 `target=local`）
- `requirement` 写清：页面（如登录页）+ 删除/隐藏哪一块（抄【截图理解】里的标注描述）+ 验收（该横幅/区块不再出现）
- 已有 MES/工程目录时不要问技术栈；`workspace` 能填则填，否则留空由确认卡补

...
| ID | 功能 | 说明 | 优先级 |
|----|------|------|--------|
| A1 | 人话能力地图 | 按模块输出「能干什么 + 代表表 + 可怎么问」 | P0 |
| A2 | 场景表包 / API 包 | 固定场景（下达→入库、收料、检验）一包表或路径 | P0 |
| A3 | 单表/单实体释义 | 字段含义、关联、常见坑 | P0 |
| A4 | 术语词典 | 中英/口语别名 → 规范实体 | P0 |
| A5 | 文档与现场差异报告 | 对比 L0 与 L2 抽检，标出过期点 | P1 |
| A6 | 导出摸底报告 | Markdown/Excel 给客户或中软对齐 | P1 |

### 2.3 技术落点

- 工具：现有 schema / capability 工具链（见 [`表结构业务能力分析说明.md`](../MES业务/表结构业务能力分析说明.md)）  
...
| B5 | 指标口径包 | 如「在制」「当日完工」「紧急未完工」定义进 Skill/配置 | P1 |
| B6 | 聚合分析工具 | count/group_by 走 API 聚合或受控 SQL | P1 |
| B7 | 分析报告模板 | 一键「产线异常日报」Markdown | P2 |
| B8 | 字段级权限 | 按角色隐藏金额/客户等敏感列 | P2 |

### 3.4 技术落点

- `entities.json` + `query_platform_data` / `export_platform_data`  
- Skill：`ops-query-playbook` 扩展「分析话术」；指标口径单独配置文件（避免写死在提示词）  
...
| ID | 功能 | 说明 | 优先级 |
|----|------|------|--------|
| C1 | 运维场景清单固化 | 文档 + Skill 对齐 ≥8 条值班话术 | P0 |
| C2 | 异常单标准链 | 查询 → 汇总 → 导出 →（可选）受控更新 | P0 |
| C3 | 接口健康 | 现有沙箱探活 + 文档化「何时可对预发探活」 | P0 |
| C4 | 写操作确认与审计回看 | 已有 M2；补「谁改了什么」对话入口 | P0 |
| C5 | 故障排查树 | 无数据 / 401 / 超时 / 实体混用 → 分支提示 | P1 |
| C6 | 日志/告警只读接入 | 对接现场日志索引或告警 API（只读） | P2 |
| C7 | 变更窗口与双人确认 | 高危写操作二次确认或角色门槛 | P2 |

### 4.3 技术落点

- Skills：`ops-query-playbook`、导入确认相关 Skill  
- Middleware：实体守卫、HITL 写确认  
- 工具：`query_write_audit`、健康分析工具  
- 文档：运维值班手册（从本方案 C1 场景表生成）  
...
| D4 | 「改功能」前置清单 | 写码前强制确认：改哪些页、依赖哪些 API/表 | P1 |
| D5 | 验收话术包 | 写码结束后推荐 B 侧查询/手工点检步骤 | P1 |
| D6 | 与桌面双交付 | 安装包配置 Key/ERP 后可用写码；网页不回退 | P0（已有） |

### 5.3 技术落点

- `apps/cursor_dev`、`apps/local_dev`、IDE review 工具  
- Skill：写码澄清 / UI 设计；新增「MES 改功能前置」短清单  

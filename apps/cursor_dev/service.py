    ...
    try:
        for round_i in range(1, max_rounds + 1):
            ...
            try:
                ...
                send_payload, img_notes, img_n = build_send_message(
                    prompt,
                    list(job.get("file_paths") or []),
                ...
                if img_n:
                    _emit(
                        ...
                        {
                            "type": "status",
                            "text": f"已向 Cursor 附带 {img_n} 张原始截图（Cloud 可直接看图）",
                            "phase": "agent",
                        },
                    )
                for note in img_notes[:5]:
                    _emit(sink, {"type": "status", "text": note, "phase": "agent"})

                run = agent.send(send_payload)
                ...
                try:
                    for message in (stream_fn() if callable(stream_fn) else []):
                        ...
                        if mtype == "assistant":
                            ...
                            title = {
                                "tool_call": "Cursor 正在调用工具…",
                                "tool": "Cursor 工具执行中…",
                                "status": "Cursor 状态更新…",
                            }.get(str(mtype), f"Cursor：{mtype}")
                            # 刷新 updated_at，便于前端/运维判断并非僵死
                            try:
                                job_store.update_job(data_dir, job_id, status="running")
                            except Exception:
                                pass
                            _emit(
                                sink,
